...
const GameTabs = ({ tabs }: { tabs: GameTabItem[] }) => {
  return (
    ...
    <Tab.List className="text-white-200 text-xl w-full">
      {tabs.map((tab, idx)=>{return <Tab key={idx} className="px-2 mt-2 border-solid border-2 rounded-t-md
        bg-gradient-to-r from-green-500 to-blue-600 font-dungGeunMo">{tab.tabTitle}</Tab>})}
    </Tab.List>
    <Tab.Panels className="text-white-200">
      <Tab.Panel><GameReviewContainer/></Tab.Panel>
      <Tab.Panel><GameArticleListContainer/></Tab.Panel>
